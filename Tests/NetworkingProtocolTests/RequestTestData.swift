...
extension GetResponse {
    static let mocks: [GetResponse] = {
        guard let data = GetResponse.responseString.data(using: context.stringEncoding) else {
            fatalError("could not get data")
        }
        do {
            return try data.decode(Array<GetResponse>.self)
        } catch {
